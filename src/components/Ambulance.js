...
export default function Model({current}) {
	...
	const ambulance = useGLTF('./models/scene/ambulance.glb')
	const animations = useAnimations(ambulance.animations, ambulance.scene)

	useEffect(()=>{
		const bakedMaterial = new MeshBasicMaterial({map: bakedMap})
		let children = [...ambulance.scene.children]
		children.forEach(child=>child.material = bakedMaterial)
	}, [])

	useEffect(()=>{
		if (current!==1 && !isActive.current) return
		if (current!==1) {
			isActive.current = false
			gsap.to(ambulance.scene.scale, {x: 0.001, y: 0.001, z: 0.001, duration: 0.5})
			return
		}
		...
		if (!isAnimated.current) {
			...
			gsap.to(ambulance.scene.scale, {x: 1, y: 1, z: 1, delay: 1, duration: 0.5})
		}
	...
	return (
		<>
		<Suspense>
			<primitive object={ambulance.scene} />
		</Suspense>
		</>
	)
}